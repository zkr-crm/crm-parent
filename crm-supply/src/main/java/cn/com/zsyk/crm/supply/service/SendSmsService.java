...
//                infoList.add(memberList.get(i).get("PHONENO"));
        ...
        Date endDate=new Date(now .getTime() + 600000);
            this.smsInvoke(smsUrl, infoList,sysSmsSendGroup.getMsgTopic() ,sysSmsSendGroup.getSendObj() ,sysSmsSendGroup.getBizNo(),sysSmsSendGroup.getMsgContent(), "", sdf.format(startDate), sdf.format(endDate));
//            flag=flag+phoneNum;
//            phoneNum=phoneNum*2;
...
//        List<SysSmsSendGroup> sysSmsSendGroup = sysSmsSendGroupMapper.selectAll();
    ...
    public Map<String, Object> smsInvoke(String url, List<Map<String,String>> infoList, String msgTopic,String sendObj,String bizNo,String contents,
        ...
        if(isTest==null ||!"0".equals(isTest)){
            startTime="";//测试时，不传time，不会实际发送短信到手机上
            endTime="";
        }
        URL endpoint = null;
        try {
