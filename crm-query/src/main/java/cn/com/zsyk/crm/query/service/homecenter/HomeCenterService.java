...
public class HomeCenterService {
	...
	public List selectAllEventByUser() {
		try {
			...
			PageBean p = dao.selectPageById("cn.com.zsyk.crm.query.mapper.SysAutoRemindMapper.selectAllEventByUser",
					param, true);
			if (p != null) {
				return p.getList();
			}
	...
	public List getMyRenewalByEntity(List<String> employeeIdList) {
		try {
			...
			PageBean p = dao.selectPageById("cn.com.zsyk.crm.query.mapper.SysAutoRemindMapper.selectAllRenwalByUser",
					param, true);
			if (p != null) {
				return p.getList();
			}
