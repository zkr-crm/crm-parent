...
public class AppMsgCtrl {
	...
	public Result modAllIsReadStat(@RequestBody String data) {
		Result res = new Result();
		Map map=(Map) JSON.parse(data);
		String employeeId=(String)map.get("employeeId");
		int count = 0;
		count = remindMsgService.modAllIsReadStat(employeeId);
		if (count <= 0) {
	...
	public Result modAllMailIsReadStat(@RequestBody String data) {
		Result res = new Result();
		Map map=(Map) JSON.parse(data);
		String employeeId=(String)map.get("employeeId");
		int count = 0;

		count = remindMsgService.modAllMailIsReadStat(employeeId);
