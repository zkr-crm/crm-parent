...
public class TagDetailMngService {
	 ...
	 * 根据id获取其下所有tag
	 */
	public List<SysTagInfo> getChildTagsById( String id){
		List<SysTagInfo> retList = new ArrayList<SysTagInfo>();
		SysTag t = new SysTag();
		t.setParentTagId(id);
		List<SysTagInfo> childTags = tagDetailMapper.selectByEntity(t);
		retList.addAll(childTags);
		if( childTags != null && childTags.size() > 0 ) {
			for(SysTag item : childTags) {
				List<SysTagInfo> nextChilds = this.getChildTagsById(item.getTagId().toString());
	 ...
	 * 根据typeList获取对应所有tag
	 */
	public List<SysTagInfo> getTagsByTypes(List<SysTagType> typeList){
