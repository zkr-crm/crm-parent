package cn.com.zsyk.crm.manage.service.homecenter;
...
public class HomeCenterService {
	...
	public List selectAllEventByUser() {
		try {
			...
			param.put("receivUser", ContextContainer.getContext().getEmployeeId());
			PageBean p = dao.selectPageById("cn.com.zsyk.crm.manage.mapper.SysAppRemindMapper.selectAllEventByUser",
					param, true);
			if (p != null) {
				return p.getList();
			}
	...
	public List getMyRenewalByEntity(List<String> employeeIdList) {
		try {
			...
			PageBean p = dao.selectPageById("cn.com.zsyk.crm.manage.mapper.SysAppRemindMapper.selectAllRenwalByUser",
					param, true);
			if (p != null) {
				return p.getList();
			}
