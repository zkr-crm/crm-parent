...
import cn.com.zsyk.crm.ecif.bo.cust.*;
...
public class CustSimTaskMngService {
	...
	public PageBean selectAllTaskByEntity(SimTaskList simTask) {
		...
		if(bean!=null) {
			...
			List<EcSimTask> retlstList = bean.getList();
			for (EcSimTask ec : retlstList) {
				if (ec.getDealUser() != null && ec.getDealUser() != "") {
					ec.setUserName(agentMap.get(ec.getDealUser()));
				}
				if (ec.getApproveUser() != null && ec.getApproveUser() != "") {
					ec.setApproveUserName(agentMap.get(ec.getApproveUser()));
				}
				if (ec.getSplitDealUser() != null && ec.getSplitDealUser() != "") {
					ec.setSplitUserName(agentMap.get(ec.getSplitDealUser()));
				}
				if (ec.getSplitApproveUser() != null && ec.getSplitApproveUser() != "") {
					ec.setSplitApproveUserName(agentMap.get(ec.getSplitApproveUser()));
				}
			}
			bean.setList(retlstList);
		}
		return bean;
	}
	...
	public String getNewCustNoByOldCustNo(String custNo) throws Exception {
		String newCustNo=custNo;

		EcCustMergeRela record = new EcCustMergeRela();
		record.setOldCustNo(custNo);
		record.setMergeType("1");
		...
		if(custMergeRelaList==null || custMergeRelaList.size()==0){
			...
			String tmpNew=custMergeRelaList.get(0).getNewCustNo();
			if(tmpNew!=null && !"".equals(tmpNew)){
				newCustNo=getNewCustNoByOldCustNo(tmpNew);
			}
		}
		return newCustNo;
	}
	...
	private String custBaseInfoMerge(List<SimCustBaseInfo> mergeCustList) throws Exception {
		...
		for (SimCustBaseInfo item : mergeCustList) {
			if (flg) {
				...
				if(custPer1.getCustTyp().equals(EnumType.CustType.per_formal_cust.value)){
					custTyp = EnumType.CustType.per_formal_cust.value;
				}
			} else {
				custPer2 = custPerMapper.selectByPrimaryKey(item.getCustNo());
				if(custPer2.getCustTyp().equals(EnumType.CustType.per_formal_cust.value)){
					custTyp = EnumType.CustType.per_formal_cust.value;
				}
		...
		if(EnumType.CustType.per_formal_cust.value.equals(custTyp)){
			custInfo.setCustTyp(EnumType.CustType.per_formal_cust.value);
		}
