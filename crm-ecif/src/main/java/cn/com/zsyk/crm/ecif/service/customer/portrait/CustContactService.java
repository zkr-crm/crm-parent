...
public class CustContactService {
	...
	public  List<EcContactWay> getContactWayOneList(String custNo) {
		List<EcContactWay> ecContactWay = ecContactWayMapper.selectByPrimaryKeyList(custNo);
		return ecContactWay;
	}
...
//		}
	...
	public List<EcContactAddr> selectContactAddrByPkList(String custNo) {
		List<EcContactAddr> ecContactAddr = ecContactAddrMapper.selectByPrimaryKeyList(custNo);
		return ecContactAddr;
	}
	/**
	 * Desc: 根据客户号、地址类型获取客户联系地址
	...
	public void saveOrUpdateContactAddr(PerCustBaseInfo perCustBaseInfo) {
		...
		if(phonesList.size()!=0){
			for (EcCustPhone ecCustPhone : phonesList) {
				EcContactWay ecContactWay = this.getContactWayOne(perCustBaseInfo.getCustNo(), ecCustPhone.getId().intValue());
...
////			//其他手机 集合  add lijiangcheng 2019-08-02
//			List<EcCustPhone> phonesList = perCustBaseInfo.getPhoneList();
//			if(phonesList.size()!=0){
//				for (EcCustPhone ecCustPhone : phonesList) {
//					if (ecCustPhone.getId() != null && ecCustPhone.getId() != 0) {
//						EcCustPhone ecCustPhoneNew	=ecCustPhoneMapper.selectByPrimaryKeyAndCustNo(ecCustPhone.getId(),perCustBaseInfo.getCustNo());
//						if(null!=ecCustPhoneNew){
////							ecCustPhone1.setUpdateTime();
////							ecCustPhone1.setUpdateUser();
////							ecCustPhone1.setCustSource(EnumType.DataSource.ecif.getValue());
////							ecCustPhone1.setCustNo(perCustBaseInfo.getCustNo());
//							ecCustPhoneNew.setPhoneNo(ecCustPhone.getPhoneNo());
//							ecCustPhoneNew.setId(ecCustPhone.getId());
//							uptCustPhone(ecCustPhoneNew);
//						}
//					}
//				}
//			}
...
//			if(phonesList.size()!=0){
//				for (EcCustPhone ecCustPhone : phonesList) {
//					//ecCustPhone.setUpdateTime();
////					ecCustPhone.setUpdateUser();
//					ecCustPhone.setCustSource(EnumType.DataSource.ecif.getValue());
//					ecCustPhone.setCustNo(perCustBaseInfo.getCustNo());
//				}
//			}
			// 住宅电话 homeTel;
