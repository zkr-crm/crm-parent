...
//			}
			...
			logger.info("无客户反向关系");
			throw new ServiceException("客户["+ecCustPer.getCustName()+"]性别为："+EnumType.Sex.toEnum(ecCustPer.getSex()).desc.toString()+";客户["+ecCustPerRef.getCustName()+"]性别为："+EnumType.Sex.toEnum(ecCustPerRef.getSex()).desc.toString()+"和关系类型不符，请选择其他关系");
		}
